...
5. **野外袭击判定** — 野外袭击可命中任何玩家；若目标有护盾则本次攻击失败，结算全 0，仍记录日志并 @ 通知；野外袭击不发放护盾
6. **袭击调度** — 采用分散时间槽，避免同一时刻连发；全局保证自然 1.5 小时窗口内至少发生 1 次袭击
7. **资源溢出掉盾** — 护盾期间若“预计可掠夺资源（仓库+收集器）”超过动态阈值，会加速衰减护盾；资源越多衰减越快（含购买护盾与 PvP 护盾）
8. **防御损伤与维修** — 被玩家或野外袭击后，防御建筑会累积损伤并按比例降低防御力（可降至0）；可在商店建筑详情页或 `/clan_repair` 花金币修复
