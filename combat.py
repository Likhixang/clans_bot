...
def calculate_attack(attacker: dict, defender: dict,
                     ...
                     atk_roll: float | None = None,
                     def_roll: float | None = None) -> dict:
...
def recommend_troops(attacker: dict, defender: dict) -> dict:
    ...
    # 最后通用补充：把所有未进入 plan 的剩余可用兵种也加进来，避免遗漏
    planned_set = set(plan)
    for tid, cnt in available.items():
        if tid not in planned_set and cnt > 0:
            plan.extend([tid] * cnt)
